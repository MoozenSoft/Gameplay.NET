...
public class EventSystem
{
    ...
    private readonly Dictionary<ushort, List<(Entity, int)>> dynamicListeners = new();
...
**Interfaces:**
- Consumes: `Gameplay.Gameplay.Tags` (GameplayTag)
...
public class AbilityTaskSystem : QuerySystem<TaskStateComponent, AbilityTaskContextComponent>
{
    ...
    protected override void OnUpdate()
    {
        Query.ForEachEntity((ref TaskStateComponent state, ref AbilityTaskContextComponent ctx, Entity entity) =>
        {
            ...
            {
                // Check: are ALL tasks under this ActiveAbility Done?
                if (AllTasksDone(ctx.ActiveAbility))
                    activationSystem.CancelAbility(ctx.ActiveAbility); // or EndAbility
            }
        });
    }
}
