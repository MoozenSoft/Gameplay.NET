...
- 使用文件范围的命名空间（比如：src/Gameplay/ 目录使用 `namespace Gameplay;`，src/Gameplay/GameplayTags 目录使用 `namespace Gameplay.GameplayTags;`, samples/Gameplay.Server 目录使用 `namespace Gameplay.Server;`, tests/Gameplay.Tests/GameplayTags 目录使用 `namespace Gameplay.Tests.GameplayTags;` 之后每多一级目录命名空间就多加一级, 不加大括号缩进）
- 遵守 TDD（测试驱动开发）
