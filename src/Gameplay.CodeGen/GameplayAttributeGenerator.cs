    ...
    private static void GenerateCode(SourceProductionContext spc, ImmutableArray<FieldInfo> fields)
    {
        ...
        foreach (var kvp in groups.OrderBy(g => g.Key.Item1).ThenBy(g => g.Key.Item2))
        {
            ...
            var hintName = string.IsNullOrEmpty(ns)
                ? $"{structName}.Accessors.g.cs"
                : $"{ns}.{structName}.Accessors.g.cs";
            spc.AddSource(hintName, source);

            var regSource = GenerateRegistration(ns, structName, fieldNames, ref nextId);
            var regHint = string.IsNullOrEmpty(ns)
                ? $"{structName}.Registration.g.cs"
                : $"{ns}.{structName}.Registration.g.cs";
            spc.AddSource(regHint, regSource);
        }
    }

    /// <summary>生成 GameplayAttribute 标识符 + RegisterAll（通过 Manager 注册委托）。</summary>
    private static string GenerateRegistration(string ns, string structName,
        List<string> fieldNames, ref int nextId)
    {
        // 预分配 ID
        var ids = new int[fieldNames.Count];
        ...
        for (int i = 0; i < fieldNames.Count; i++)
        {
            sb.AppendLine($"    public static readonly Gameplay.Abilities.GameplayAttribute {fieldNames[i]} = new({ids[i]});");
            sb.AppendLine();
        }
        ...
        {
            string indent = "        ";
            sb.AppendLine($"{indent}mgr.RegisterAttribute({fieldNames[i]},");
            sb.AppendLine($"{indent}    (Entity entity, out float value) =>");
            sb.AppendLine($"{indent}    {{");
            sb.AppendLine($"{indent}        ref var data = ref entity.GetComponent<{structName}>().{fieldNames[i]};");
            sb.AppendLine($"{indent}        value = data.BaseValue;");
            sb.AppendLine($"{indent}    }},");
            sb.AppendLine($"{indent}    (Entity entity, out float value) =>");
            sb.AppendLine($"{indent}    {{");
            sb.AppendLine($"{indent}        ref var data = ref entity.GetComponent<{structName}>().{fieldNames[i]};");
            sb.AppendLine($"{indent}        value = data.CurrentValue;");
            sb.AppendLine($"{indent}    }},");
            sb.AppendLine($"{indent}    (Entity entity, float value) =>");
            sb.AppendLine($"{indent}    {{");
            sb.AppendLine($"{indent}        ref var data = ref entity.GetComponent<{structName}>().{fieldNames[i]};");
            sb.AppendLine($"{indent}        data.CurrentValue = value;");
            sb.AppendLine($"{indent}    }});");
            sb.AppendLine();
        }
        sb.AppendLine($"    }}");
