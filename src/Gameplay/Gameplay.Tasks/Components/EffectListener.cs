...
/// <summary>
/// GE 监听能力——事件驱动：EffectSystem 施加/移除 GE 时，匹配 <see cref="Query"/> 的 Task 完成（Done）。<br/>
/// 对应 UE 的 WaitGameplayEffectApplied / WaitGameplayEffectRemoved。<br/>
/// 注意：<see cref="Query"/> 是可变 class 引用——调用者不得在 Task 存活期间修改它（Task 条件应保持创建时快照语义）。
/// </summary>
