...
/// <summary>计时器能力 Driver——每 <see cref="TimerComponent.Interval"/> 秒发一次脉冲事件，剩余脉冲数归零后 Task 完成（Done）。</summary>
/// <remarks>
/// <para>对应 UE 的 Repeat（周期性通知外部）。</para>
/// <para>Pending 帧不累积（与 DelaySystem 一致——注册帧不推进）；每帧至多 1 次脉冲（大 dt 不风暴，残留累积下一帧不丢）。</para>
/// <para>RemainingPulses=0 = 无限脉冲（永不完成）。</para>
/// </remarks>
